...
export const TwelveCellsInput = ({
    ...
    const twelveRef = useRef<any>(null);
    ...
    useMemo(() => {
        if (currentValue) {
            ...
            setTwelveValue(currentValue[11] ?? '')
        }
    }, [currentValue])

    useMemo(() => {
        setCurrentValue(`${firstValue + secondValue + thirdValue + fourthValue + fiveValue + sixValue + sevenValue + eightValue + nineValue + tenValue + elevenValue + twelveValue}`)
    }, [firstValue, secondValue, thirdValue, fourthValue, fiveValue, sixValue, sevenValue, eightValue, nineValue, tenValue, elevenValue, twelveValue])
    ...
    useMemo(() => {
        switch (currentIndex) {
            ...
            case 10:
                ...
                break
            case 11:
                if (twelveRef) twelveRef?.current?.focus()
                break
            default:
                break
        }
    ...
    const onKeyDownHandler = (e: any) => {
        if (((e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode >= 96 && e.keyCode <= 105)) && currentIndex < 11) setCurrentIndex(currentIndex + 1)
    }
    ...
    return (
        <div className={classNames(s.cellInputWrapper)}>
            ...
            <div className={s.cells}>
                ...
                <input inputMode={"numeric"} maxLength={1} ref={twelveRef} className={s.input} data-index={11} type="text" value={twelveValue} onChange={(e) => onChangeHandler(e, setTwelveValue)} onKeyDown={onKeyDownHandler} onFocus={onFocusHandler}/>
            </div>
        </div>
    );
};
